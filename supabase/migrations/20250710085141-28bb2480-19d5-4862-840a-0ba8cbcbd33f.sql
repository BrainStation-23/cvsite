...
-- Update the get_resource_planning_data function to exclude completed engagements
...
BEGIN
  ...
  -- Calculate total resource planning records count (unfiltered, excluding completed engagements)
  SELECT COUNT(*)
  INTO total_count
  FROM resource_planning rp
  WHERE rp.engagement_complete = false;
  
  -- Count filtered results with proper UUID comparisons, excluding completed engagements
  ...
  -- Build resource planning query with proper sorting and pagination, excluding completed engagements
