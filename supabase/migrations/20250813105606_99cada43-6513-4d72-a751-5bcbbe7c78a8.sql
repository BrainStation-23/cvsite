...
-- Fix the bulk sync function to resolve ambiguous column references
...
BEGIN
  ...
  LOOP
    ...
    IF existing_profiles_map ? employee_id THEN
      ...
      -- Update existing profile with explicit table alias
      BEGIN
        UPDATE profiles p
        SET 
          email = COALESCE(user_email, p.email),
          first_name = COALESCE(user_first_name, p.first_name),
          manager = manager_id,
          sbu_id = sbu_id,
          updated_at = now()
        WHERE p.employee_id = employee_item->>'employeeId';
