...
-- Update get_planned_resource_data function with project_level and project_type_name
...
BEGIN
  ...
  -- Get filtered count and build result in one query
  WITH filtered_data AS (
    ...
    WHERE rp.engagement_complete = false
      ...
      -- Project search
      AND (project_search IS NULL OR project_search = '' OR pm.id::text = project_search OR pm.project_name ILIKE '%' || project_search || '%')
  ...
  paginated_data AS (
    ...
    ORDER BY
      ...
      -- Employee ID sorting by numeric part only
      CASE WHEN sort_by = 'employee_id' AND sort_order = 'asc' THEN 
        COALESCE(
          NULLIF(regexp_replace(employee_id, '[^0-9]', '', 'g'), '')::integer,
          0
        ) 
      END ASC,
      CASE WHEN sort_by = 'employee_id' AND sort_order = 'desc' THEN 
        COALESCE(
          NULLIF(regexp_replace(employee_id, '[^0-9]', '', 'g'), '')::integer,
          0
        ) 
  ...
  SELECT 
    ...
    json_build_object(
            ...
            'project', CASE 
              WHEN pd.project_id IS NOT NULL THEN 
                json_build_object(
                  ...
                  'budget', pd.budget,
                  'project_level', pd.project_level,
                  'project_type_name', pd.project_type_name
                )
              ELSE NULL 
            END
          )
      ...
      'pagination', json_build_object(
        ...
        'filtered_count', (SELECT COUNT(*) FROM filtered_data),
        'page', page_number,
        'per_page', items_per_page,
        'page_count', CEIL((SELECT COUNT(*) FROM filtered_data)::numeric / items_per_page)
      )
    )
  INTO filtered_record_count, results_json;
  
  RETURN results_json;
END;
$function$;

-- Update get_weekly_validation_data function with project_level and project_type_name
...
DECLARE
  ...
  results_json json;
BEGIN
  -- Get total count of all resource planning records that need weekly validation
  SELECT COUNT(*)
  INTO total_count
  FROM resource_planning
  WHERE weekly_validation = false AND engagement_complete = false;
