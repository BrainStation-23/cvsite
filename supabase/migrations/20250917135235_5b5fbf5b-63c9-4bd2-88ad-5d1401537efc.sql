-- Update the manage_non_billed_sync_cron function to call RPC directly
...
BEGIN
  ...
  -- Create new cron job if enabled (directly call RPC function)
  IF p_enabled THEN
    SELECT cron.schedule(
      job_name_val,
      p_schedule,
      'SELECT sync_non_billed_resources_now();'
    ) INTO job_id;
  END IF;
