...
-- Update the RPC function to handle weekly validation filtering at the database level
CREATE OR REPLACE FUNCTION public.get_comprehensive_resource_planning_data(
  ...
  weekly_validation_filter boolean DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Unplanned resources filter
  IF include_unplanned THEN
    where_conditions := array_append(where_conditions, 'rp.project_id IS NULL');
  ELSE
    -- For planned resources, exclude those without projects
    where_conditions := array_append(where_conditions, 'rp.project_id IS NOT NULL');
  END IF;

  -- Weekly validation filter
  IF include_weekly_validation AND weekly_validation_filter IS NOT NULL THEN
    where_conditions := array_append(where_conditions, 'rp.weekly_validation = ' || weekly_validation_filter);
  END IF;
