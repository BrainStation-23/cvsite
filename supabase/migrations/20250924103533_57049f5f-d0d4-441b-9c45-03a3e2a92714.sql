-- Update list_users RPC function to use custom roles instead of old role system
CREATE OR REPLACE FUNCTION public.list_users(
  search_query text DEFAULT NULL::text,
  filter_custom_role_id uuid DEFAULT NULL::uuid,
  filter_sbu_id uuid DEFAULT NULL::uuid,
  filter_manager_id uuid DEFAULT NULL::uuid,
  filter_resource_type_id uuid DEFAULT NULL::uuid,
  filter_expertise_id uuid DEFAULT NULL::uuid,
  filter_min_total_years numeric DEFAULT NULL::numeric,
  filter_max_total_years numeric DEFAULT NULL::numeric,
  filter_min_company_years numeric DEFAULT NULL::numeric,
  filter_max_company_years numeric DEFAULT NULL::numeric,
...
BEGIN
  ...
  LEFT JOIN sbus sbu_context ON ur.sbu_context = sbu_context.id
  ...
  AND (filter_custom_role_id IS NULL OR ur.custom_role_id = filter_custom_role_id)
  ...
  SELECT json_agg(
    json_build_object(
      ...
      'sbu_context', ur.sbu_context,
      'sbu_context_name', sbu_context.name,
  ...
  LEFT JOIN resource_types rt ON p.resource_type = rt.id
  LEFT JOIN expertise_types et ON p.expertise = et.id
  WHERE au.row_num BETWEEN (page_number - 1) * items_per_page + 1 AND page_number * items_per_page;
