...
-- Fix the get_employee_profiles function to resolve the GROUP BY issue
...
BEGIN
  ...
  -- Count filtered results with all the search conditions
  ...
  -- Build profiles query returning only table view data (without DISTINCT to avoid GROUP BY issues)
  ...
  FROM (
    SELECT DISTINCT
      ...
      p.created_at,
      p.updated_at,
      -- General information for display
      json_build_object(
        'first_name', COALESCE(gi.first_name, ''),
        'last_name', COALESCE(gi.last_name, ''),
        'biography', gi.biography,
        'profile_image', gi.profile_image,
        'current_designation', gi.current_designation
      ) as general_information,
      -- Technical skills (limited for table view) - separate subquery to avoid GROUP BY issues
      (
        SELECT json_agg(json_build_object(
          'id', ts_sub.id,
          'name', ts_sub.name,
          'proficiency', ts_sub.proficiency
        ))
        FROM (
          SELECT ts.id, ts.name, ts.proficiency
          FROM technical_skills ts
          WHERE ts.profile_id = p.id
          ORDER BY COALESCE(ts.priority, 999) ASC, ts.name ASC
          LIMIT 10
        ) ts_sub
      ) AS technical_skills,
      -- Specialized skills (limited for table view) - separate subquery to avoid GROUP BY issues
      (
        SELECT json_agg(json_build_object(
          'id', ss_sub.id,
          'name', ss_sub.name,
          'proficiency', ss_sub.proficiency
        ))
        FROM (
          SELECT ss.id, ss.name, ss.proficiency
          FROM specialized_skills ss
          WHERE ss.profile_id = p.id
          ORDER BY COALESCE(ss.priority, 999) ASC, ss.name ASC
          LIMIT 10
        ) ss_sub
      ) AS specialized_skills,
      -- Recent trainings (limited for table view)
      (
        SELECT json_agg(json_build_object(
          'id', tr.id,
          'title', tr.title,
    ...
    LEFT JOIN general_information gi ON p.id = gi.profile_id
    WHERE p.id IN (
      -- Apply all the same filters as above for the filtered profiles
      SELECT DISTINCT p2.id
      FROM profiles p2
      LEFT JOIN general_information gi2 ON p2.id = gi2.profile_id
      LEFT JOIN technical_skills ts2 ON p2.id = ts2.profile_id
      LEFT JOIN specialized_skills ss2 ON p2.id = ss2.profile_id
      LEFT JOIN experiences ex2 ON p2.id = ex2.profile_id
      LEFT JOIN education ed2 ON p2.id = ed2.profile_id
      LEFT JOIN universities u2 ON ed2.university = u2.name OR ed2.university ILIKE '%' || u2.acronyms || '%'
      LEFT JOIN trainings tr2 ON p2.id = tr2.profile_id
      LEFT JOIN achievements ac2 ON p2.id = ac2.profile_id
      LEFT JOIN projects pr2 ON p2.id = pr2.profile_id
      WHERE (
        search_query IS NULL
        OR p2.first_name ILIKE '%' || search_query || '%'
        OR p2.last_name ILIKE '%' || search_query || '%'
        OR p2.employee_id ILIKE '%' || search_query || '%'
        OR p2.email ILIKE '%' || search_query || '%'
        OR gi2.first_name ILIKE '%' || search_query || '%'
        OR gi2.last_name ILIKE '%' || search_query || '%'
        OR gi2.current_designation ILIKE '%' || search_query || '%'
        OR gi2.biography ILIKE '%' || search_query || '%'
        OR EXISTS (
          SELECT 1 FROM technical_skills 
          WHERE profile_id = p2.id AND name ILIKE '%' || search_query || '%'
        )
        OR EXISTS (
          SELECT 1 FROM specialized_skills 
          WHERE profile_id = p2.id AND name ILIKE '%' || search_query || '%'
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1 FROM technical_skills 
          WHERE profile_id = p2.id AND name ILIKE '%' || skill_filter || '%'
        )
        OR EXISTS (
          SELECT 1 FROM specialized_skills 
          WHERE profile_id = p2.id AND name ILIKE '%' || skill_filter || '%'
      ...
      AND (
        ...
        OR (
          completion_status = 'complete' 
          AND EXISTS (SELECT 1 FROM technical_skills ts_comp WHERE ts_comp.profile_id = p2.id UNION SELECT 1 FROM specialized_skills ss_comp WHERE ss_comp.profile_id = p2.id)
          AND EXISTS (SELECT 1 FROM experiences ex_comp WHERE ex_comp.profile_id = p2.id)
          AND EXISTS (SELECT 1 FROM education ed_comp WHERE ed_comp.profile_id = p2.id)
          AND EXISTS (SELECT 1 FROM projects pr_comp WHERE pr_comp.profile_id = p2.id)
        )
        OR (
          completion_status = 'incomplete'
          AND NOT (
            EXISTS (SELECT 1 FROM technical_skills ts_comp WHERE ts_comp.profile_id = p2.id UNION SELECT 1 FROM specialized_skills ss_comp WHERE ss_comp.profile_id = p2.id)
            AND EXISTS (SELECT 1 FROM experiences ex_comp WHERE ex_comp.profile_id = p2.id)
            AND EXISTS (SELECT 1 FROM education ed_comp WHERE ed_comp.profile_id = p2.id)
            AND EXISTS (SELECT 1 FROM projects pr_comp WHERE pr_comp.profile_id = p2.id)
          )
        ...
        OR (
          completion_status = 'no-skills'
          AND NOT EXISTS (SELECT 1 FROM technical_skills ts_comp WHERE ts_comp.profile_id = p2.id UNION SELECT 1 FROM specialized_skills ss_comp WHERE ss_comp.profile_id = p2.id)
        )
        OR (
          completion_status = 'no-experience'
          AND NOT EXISTS (SELECT 1 FROM experiences ex_comp WHERE ex_comp.profile_id = p2.id)
        )
        OR (
          completion_status = 'no-education'
          AND NOT EXISTS (SELECT 1 FROM education ed_comp WHERE ed_comp.profile_id = p2.id)
        )
      )
    )
    ORDER BY
