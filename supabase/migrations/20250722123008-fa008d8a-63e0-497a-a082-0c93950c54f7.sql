...
-- Update the get_employee_profiles function to include additional profile data
...
BEGIN
  ...
  -- Build the main query with comprehensive filtering
  ...
  FROM (
    ...
    FROM profiles p
    INNER JOIN filtered_profiles fp ON p.id = fp.id
    LEFT JOIN general_information gi ON p.id = gi.profile_id
    LEFT JOIN expertise_types et ON p.expertise = et.id
    ORDER BY
