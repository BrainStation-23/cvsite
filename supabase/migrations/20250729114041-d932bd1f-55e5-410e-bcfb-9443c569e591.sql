...
-- Create a new RPC function for weekly validation data
CREATE OR REPLACE FUNCTION public.get_weekly_validation_data(search_query text DEFAULT NULL::text, page_number integer DEFAULT 1, items_per_page integer DEFAULT 10, sort_by text DEFAULT 'created_at'::text, sort_order text DEFAULT 'desc'::text, sbu_filter text DEFAULT NULL::text, manager_filter text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- Calculate total resource planning records count (unfiltered, excluding completed engagements and only weekly_validation = false)
  SELECT COUNT(*)
  INTO total_count
  FROM resource_planning rp
  WHERE rp.engagement_complete = false 
  AND rp.weekly_validation = false;
  
  -- Count filtered results with proper UUID comparisons, excluding completed engagements and only weekly_validation = false
  ...
  -- Build resource planning query with proper sorting and pagination, excluding completed engagements and only weekly_validation = false
