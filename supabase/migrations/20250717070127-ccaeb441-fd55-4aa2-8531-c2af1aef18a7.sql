...
-- Update the get_employee_profiles function to be more focused and secure
...
BEGIN
  ...
  -- Build simplified profiles query returning only table view data
  ...
  FROM (
    SELECT 
      ...
      -- Technical skills (limited for table view)
      (
      ...
      -- Specialized skills (limited for table view)
      (
        SELECT json_agg(json_build_object(
          'id', ss.id,
          'name', ss.name,
        ...
        ORDER BY ss.priority ASC, ss.name ASC
        LIMIT 10
