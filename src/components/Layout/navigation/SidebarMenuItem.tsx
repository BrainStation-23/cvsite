    ...
    return (
      <>
        <div className="flex items-center">
          ...
          >
            {React.cloneElement(item.icon as React.ReactElement, { className: 'w-5 h-5 text-teal-400 group-hover:text-teal-200' })}
          ...
          <ul className="ml-4 mt-1">
            {item.submenuItems?.map((subItem) => (
              <li key={subItem.to} className="mb-1">
                ...
                >
                  {React.cloneElement(subItem.icon as React.ReactElement, { className: 'text-teal-400 group-hover:text-teal-200' })}
