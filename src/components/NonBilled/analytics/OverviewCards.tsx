...
export function OverviewCards({ data, isLoading }: OverviewCardsProps) {
  ...
  const cards = [
    ...
    {
      title: 'Long Term',
      value: data.overview.long_term_bench_count,
      icon: Calendar,
      description: '>30 days on bench',
      variant: data.overview.long_term_bench_count > 0 ? 'destructive' as const : 'default' as const,
    },
    {
      title: 'Critical Risk',
      value: data.overview.critical_bench_count,
      icon: AlertTriangle,
      description: '>90 days on bench',
      variant: data.overview.critical_bench_count > 0 ? 'destructive' as const : 'default' as const,
    },
    {
      title: 'New (7 days)',
      value: data.recent_trends.new_bench_last_7_days,
      icon: TrendingUp,
