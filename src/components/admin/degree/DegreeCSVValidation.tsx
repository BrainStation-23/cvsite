...
interface DegreeCSVValidationProps {
  validationResult: {
  ...
  onProceed: (validDegrees: DegreeFormData[]) => void;
  onClose: () => void;
  isBulkImporting: boolean;
}

const DegreeCSVValidation: React.FC<DegreeCSVValidationProps> = ({
  validationResult,
  onProceed,
  onClose,
  isBulkImporting
      ...
      <div className="space-y-4">
        <div className="flex gap-4">
          <Badge variant="outline" className="text-green-600 border-green-600">
            {validationResult.valid.length} Valid Degrees
          </Badge>
          ...
          <div className="space-y-2">
            <div className="bg-green-50 border border-green-200 rounded-md p-3">
              <div className="text-green-800 font-medium">
                {validationResult.valid.length} degrees are ready to import
              </div>
              <div className="text-green-700 text-sm mt-1">
                These degrees passed all validation checks and can be safely imported.
              </div>
            </div>
            ...
            >
              {isBulkImporting ? "Importing..." : `Import ${validationResult.valid.length} Valid Degrees`}
            </Button>
          </div>
        )}
      </div>
...
const getFixSuggestion = (field: string, message: string): string => {
  if (field === 'name') {
    if (message.includes('required')) {
      return 'Action: Add a degree name in this cell';
    }
    if (message.includes('duplicate')) {
      return 'Action: Choose a different degree name or remove the duplicate row';
    }
    if (message.includes('already exists')) {
      return 'Action: This degree already exists in the database. Remove this row or use a different name';
    }
  }
  
  if (field === 'full_form') {
    return 'Action: This field is optional but should contain the full degree name if provided';
  }
  
  return 'Action: Please review and correct the value in this field';
};

export default DegreeCSVValidation;
