  ...
  return <div className="space-y-2">
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !startDate && !endDate && "text-muted-foreground")}>
          ...
          <div className="flex">
            ...
            <div className="border-r p-3 space-y-1 min-w-[120px]">
              ...
              <div className="space-y-1">
                {DATE_PRESETS.map(preset => <Button key={preset.label} variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => handlePresetClick(preset)}>
                    {preset.label}
                  </Button>)}
              </div>
              <Separator className="my-2" />
              <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8 text-muted-foreground" onClick={handleClearRange}>
                Clear
              </Button>
            </div>
            ...
            <div className="p-3">
              ...
              <div className="flex gap-3">
                <div>
                  ...
                  <Calendar mode="single" selected={tempStartDate} onSelect={setTempStartDate} className="p-0" />
                </div>
                <div>
                  <Label className="text-xs">End Date</Label>
                  <Calendar mode="single" selected={tempEndDate} onSelect={setTempEndDate} className="p-0" />
                </div>
