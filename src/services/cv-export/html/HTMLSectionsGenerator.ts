...
export class HTMLSectionsGenerator {
  ...
  generateSectionsHTMLWithPageBreaks(sections: any[], profile: any, fieldMappings: any[]): string {
    if (!sections || sections.length === 0) {
      return '<div class="cv-page"><div class="no-sections">No sections configured</div></div>';
    }
    ...
    for (const section of sortedSections) {
      console.log('=== SECTIONS GENERATION WITH PAGE BREAKS DEBUG ===');
      
      if (section.section_type === 'page_break') {
        // Close current page and start new one
        if (currentPageContent.trim()) {
          html += `<div class="cv-page">${currentPageContent}</div>`;
          currentPageContent = '';
          isFirstPage = false;
        }
        continue;
      ...
      switch (section.section_type) {
        case 'experience':
          sectionHTML = this.generateExperienceSection(profile, fieldMappings);
          break;
        case 'education':
          sectionHTML = this.generateEducationSection(profile, fieldMappings);
          break;
        case 'technical_skills':
          sectionHTML = this.generateTechnicalSkillsSection(profile, fieldMappings);
          break;
        case 'specialized_skills':
          sectionHTML = this.generateSpecializedSkillsSection(profile, fieldMappings);
          break;
        case 'projects':
          sectionHTML = this.generateProjectsSection(profile, fieldMappings, section);
          break;
        case 'achievements':
          sectionHTML = this.generateAchievementsSection(profile, fieldMappings);
          break;
        case 'trainings':
          sectionHTML = this.generateTrainingsSection(profile, fieldMappings);
          break;
        default:
          console.log(`Unknown section type: ${section.section_type}`);
          continue;
    ...
    if (currentPageContent.trim() || isFirstPage) {
      html += `<div class="cv-page">${currentPageContent}</div>`;
    }

    // Ensure at least one page exists
    if (!html.trim()) {
      html = '<div class="cv-page"><div class="no-sections">No sections configured</div></div>';
    }
